...
def plot_distance_dist(intra, inter, taxa, folder):
    ...
    p = (ggplot(df, aes(x='variable', y='Distance', fill='variable')) + geom_boxplot(alpha=.7) + theme(
        axis_text_x=element_text(face="bold"), plot_title=element_text(face="bold", hjust=0.5), axis_title_x = element_blank(), axis_title_y = element_text(face = "bold"), legend_position = "bottom", legend_title = element_blank()) + labs(title=taxa))
